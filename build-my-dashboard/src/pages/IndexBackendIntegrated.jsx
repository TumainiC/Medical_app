...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const handleHealthDataError = (event) => {
      setError(event.detail);
      setLoading(false);
    };

    window.addEventListener('healthDataUpdated', handleHealthDataUpdate);
    window.addEventListener('healthDataError', handleHealthDataError);
    ...
    return () => {
      ...
      window.removeEventListener('healthDataError', handleHealthDataError);
    };
  }, []);
          ...
          {healthData && (
            <div className="glass-card rounded-3xl p-6 md:col-span-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <div className="text-2xl font-bold">{healthData.statistics?.total_records || 0}</div>
                  <div className="text-xs text-muted-foreground">Total Records</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-destructive">{healthData.statistics?.anomaly_count || 0}</div>
                  <div className="text-xs text-muted-foreground">Anomalies</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-primary">{healthData.statistics?.avg_health_score?.toFixed(1) || 0}</div>
